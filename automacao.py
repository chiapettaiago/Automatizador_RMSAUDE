from numbers import Integral
import pyautogui as py
import pyautogui
import pytesseract as ocr
from PIL import Image
import PIL.Image
import time
import pygetwindow
import cx_Oracle
import tkinter as tk
from tkinter import messagebox
from tkinter import *
from tkinter import ttk
import locale
import cv2
import numpy as np
import logging

#Instanciar arquivo de logs no sistema
logging.basicConfig(filename='logs/atividade.log', level=logging.DEBUG)

dsn = cx_Oracle.makedsn('#########', #####, '#######')
user = '##'
password = '###############'

dados_conexao = cx_Oracle.connect(user, password, dsn)

logging.debug("Application started")

#Instanciando Tkinter e criando interface de usuário
window = tk.Tk()
window.title('RPA by Iago Chiapetta')
window.geometry('400x300')
#Recurso que impede que o usuário redimensione a janela
window.resizable(width=False, height=False)
w = Label(window, text='Escolha entre as opções abaixo: ', font=35)
w.place(x=90, y=10)

Checkbutton1 = IntVar()
Checkbutton2 = IntVar()

#Recurso que impossibilita que dois CheckButtons sejam selecionados ao mesmo tempo.
def clear2():
    Checkbutton2.set(0)

def clear1():
    Checkbutton1.set(0)

def vazio():
    messagebox.showwarning("Alerta de Campos Vazios", "Você precisa preencher todos os campos para iniciar o processamento.")

#CheckBox da Interface do App 
btn1 = Checkbutton(window, text="Baixa", variable=Checkbutton1, onvalue=1, offvalue=0, height=2, width=10, command=clear2)
btn2 = Checkbutton(window, text="Perda", variable=Checkbutton2, onvalue=1, offvalue=0, height=2, width=20, command=clear1)


btn1.pack_forget()#x=105, y=55)
btn2.place(x=116, y=85)

textoPeriodo = Label(window, text="Período: ", bg="#d3d3d3")
textoPeriodo.place(x=10, y=200)
entradaPeriodoMes = Entry(window)
entradaPeriodoMes.place(x=60, y=200, width=15)
barra = Label(window, text="/", bg="#d3d3d3")
barra.place(x=75, y=200)
entradaPeriodoAno = Entry(window)
entradaPeriodoAno.place(x=85, y=200, width=30)

conexao = cx_Oracle.connect(user, password, dsn)
cursor = conexao.cursor()
cursor.execute(f"SELECT SIGLA FROM SZCADGERAL WHERE CODCOLIGADA = 1.000000 AND CLASSIFICACAO = 'C' AND SITUACAO = 'A' AND TIPOFATURAMENTO = 1 AND CODGERAL IN (SELECT CODCOMPRADOR CODGERAL FROM SZFATURACONVENIO WHERE CODCOLIGADA = 1.000000 AND (TO_CHAR(DATAGERACAO,'MM')|| '/' || TO_CHAR(DATAGERACAO,'YYYY')) = '{entradaPeriodoMes.get()}/{entradaPeriodoAno.get()}') ORDER BY SIGLA ")
lista_planos = []
row = cursor.fetchone()
while row:
    lista_planos.append(row[0])
    row = cursor.fetchone()

textoConvenio = Label(window, text="Convenio: ", bg="#d3d3d3")
textoConvenio.place(x=150, y=200)
caixaSelecao = ttk.Combobox(window, 
values=[
    "AMIL",
    "ASSIM",
    "BRADESCO",
    "CAC",
    "CEF",
    "CORPO DE BOMBEIROS DO ESTADO DO RIO DE JANEIRO",
    "FUSMA",
    "GOLDEN CROSS",
    "MEDISERVICE",
    "PETROBRAS",
    "POSTAL SAUDE",
    "SUL AMERICA",
    "UNIMED"
]
)
caixaSelecao.place(x=210, y=200, width=170)

labelFatura = Label(window, text="Ordem da Fatura: ", bg="#d3d3d3")
labelFatura.place(x=10 ,y=240)
entradaOrdemFatura = Entry(window)
entradaOrdemFatura.place(x=110, y=240, width=30)

textoUnidade = Label(window, text="Unidade: ", bg="#d3d3d3")
textoUnidade.place(x=160, y=240)
caixaUnidade = ttk.Combobox(window, values=[
    "AMBULATÓRIO (FATURAMENTO)",
    "HOSPITAL DAS CLÍNICAS DE TERESÓPOLIS",
    "MATERNIDADE DO HCTCO (FATURAMENTO)"
])
caixaUnidade.place(x=211, y=240, width=170)


#Aviso de recurso não disponível
def baixa():
    messagebox.showwarning("Recurso Futuro", "O Recurso Selecionado Ainda está sendo desenvolvido e estará disponível em breve. Fique tranquilo, nós avisaremos.")


#dados_conexao = ('DRIVER={Devart ODBC Driver for Oracle};Host=10.1.0.131;Port=1521;SERVICENAME=DBTOTVSP;UID=rm;Password=f/M701iv_LoAE1@;Direct=True')

#Rotina de automação a ser executada
def automacao():
    inicio = time.time()
    py.alert("Atenção! A automação está sendo iniciada.")
    py.PAUSE = 1.2
    res = py.size()
    if res.width == 1366 and res.height == 768:
        try:
            #Acessar onde ficam os dados a serem obtido
            py.moveTo(784, 26)
            py.click()
            py.moveTo(782, 142)
            py.click()
            title = "Controle de Recebimentos"
            window = pygetwindow.getWindowsWithTitle(title)[0]
            contador1 = 0
            if window.title == "Controle de Recebimentos (RDESKAMB.UNIFESO.LAN)":
                contador1 = contador1 + 1
                py.moveTo(573, 697)
                py.click()
                title = "TOTVS Hospitalar"
                window = pygetwindow.getWindowsWithTitle(title)[0]
                if window.title == "TOTVS Hospitalar (RDESKAMB.UNIFESO.LAN)":
                    py.press('enter')
                #Selecionar mês dos resultados
                py.moveTo(325, 392)
                py.click()
                py.write(entradaPeriodoMes.get() + entradaPeriodoAno.get())
                py.moveTo(525, 396)
                py.click()
                #Convenio
                if caixaSelecao.get() == "AMIL":
                    codPlano = 5 
                    py.moveTo(589, 416)
                    py.click()
                elif caixaSelecao.get() == "ASSIM":
                    codPlano = 7
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "BR DISTRIBUIDORA" and entradaPeriodoMes.get() == '01' and entradaPeriodoAno.get() == '2020':
                    codPlano = 1678
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "BRADESCO":
                    codPlano = 27
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                    if entradaPeriodoMes.get() == '01' and entradaPeriodoAno.get() == '2020':
                        py.press('down')
                elif caixaSelecao.get() == "CAC":
                    codPlano = 10
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "CEF":
                    codPlano = 1151
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "CORPO DE BOMBEIROS DO ESTADO DO RIO DE JANEIRO":
                    codPlano = 2879
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "FUSMA":
                    codPlano = 5383
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "GOLDEN CROSS":
                    codPlano = 20
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "MEDISERVICE":
                    codPlano = 24
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "PETROBRAS":
                    codPlano = 26
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "POSTAL SAUDE":
                    codPlano = 4113
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "SUL AMERICA":
                    codPlano = 31
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    if entradaPeriodoMes.get() == '02' and entradaPeriodoAno.get() == '2021':
                        py.press('up')
                    elif entradaPeriodoMes.get() == '11' and entradaPeriodoAno.get() == '2021':
                        py.press('down')
                    elif entradaPeriodoMes.get() == '02' and entradaPeriodoAno.get() == '2022':
                        py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "UNIMED":
                    codPlano = '4131'
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    if entradaPeriodoMes.get() == '11' and entradaPeriodoAno.get() == '2021':
                        py.press('down')
                        py.press('down')
                    elif entradaPeriodoMes.get() == '02' and entradaPeriodoAno.get() == '2022':
                        py.press('down')
                        py.press('down')
                    py.press('enter')

                py.moveTo(699, 388)
                py.click()
                if caixaUnidade.get() == "HOSPITAL DAS CLÍNICAS DE TERESÓPOLIS":
                    codHospital = 3
                    py.moveTo(836, 427)
                    py.click()
                elif caixaUnidade.get() == "AMBULATÓRIO (FATURAMENTO)":
                    codHospital = 13
                    py.moveTo(802, 410)
                    py.click()
                elif caixaUnidade.get() == "MATERNIDADE DO HCTCO (FATURAMENTO)":
                    codHospital = 14
                    py.moveTo(819, 438)
                    py.click()

                py.moveTo(914, 393)
                py.click()

            else:
                py.alert("A janela de Recebimentos não startou. Encerrando...")
                exit()

        except KeyboardInterrupt as kex:
            print("Programa encerrado pelo usuário via teclado.")
            exit()
        except:
            print("Resolução de tela não cadastrada no sistema")
            exit()
    elif res.width == 1920 and res.height == 1080:
        try:
            #Acessar onde ficam os dados a serem obtido
            py.moveTo(784, 26)
            py.click()
            py.moveTo(782, 142)
            py.click()
            title = "Controle de Recebimentos"
            window = pygetwindow.getWindowsWithTitle(title)[0]
            contador1 = 0
            if window.title == "Controle de Recebimentos (RDESKAMB.UNIFESO.LAN)":
                contador1 = contador1 + 1
                py.moveTo(770, 851)
                py.click()
                title = "TOTVS Hospitalar"
                window = pygetwindow.getWindowsWithTitle(title)[0]
                if window.title == "TOTVS Hospitalar (RDESKAMB.UNIFESO.LAN)":
                    py.press('enter')
                #Selecionar mês dos resultados
                py.moveTo(602, 550)
                py.click()
                py.write(entradaPeriodoMes.get() + entradaPeriodoAno.get())
                py.moveTo(903, 553)
                py.click()
                #Convenio
                if caixaSelecao.get() == "AMIL":
                    codPlano = 5 
                    py.moveTo(860, 570)
                    py.click()
                elif caixaSelecao.get() == "ASSIM":
                    codPlano = 7
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "BR DISTRIBUIDORA" and entradaPeriodoMes.get() == '01' and entradaPeriodoAno.get() == '2020':
                    codPlano = 1678
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "BRADESCO":
                    codPlano = 27
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                    if entradaPeriodoMes.get() == '01' and entradaPeriodoAno.get() == '2020':
                        py.press('down')
                elif caixaSelecao.get() == "CAC":
                    codPlano = 10
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "CEF":
                    codPlano = 1151
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "CORPO DE BOMBEIROS DO ESTADO DO RIO DE JANEIRO":
                    codPlano = 2879
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "FUSMA":
                    codPlano = 5383
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "GOLDEN CROSS":
                    codPlano = 20
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "MEDISERVICE":
                    codPlano = 24
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "PETROBRAS":
                    codPlano = 26
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "POSTAL SAUDE":
                    codPlano = 4113
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "SUL AMERICA":
                    codPlano = 31
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    if entradaPeriodoMes.get() == '02' and entradaPeriodoAno.get() == '2021':
                        py.press('up')
                    elif entradaPeriodoMes.get() == '11' and entradaPeriodoAno.get() == '2021':
                        py.press('down')
                    py.press('enter')
                elif caixaSelecao.get() == "UNIMED":
                    codPlano = '4131'
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    py.press('down')
                    if entradaPeriodoMes.get() == '11' and entradaPeriodoAno.get() == '2021':
                        py.press('down')
                        py.press('down')
                    elif entradaPeriodoMes.get() == '02' and entradaPeriodoAno.get() == '2022':
                        py.press('down')
                        py.press('down')
                    py.press('enter')

                py.moveTo(1143, 551)
                py.click()
                if caixaUnidade.get() == "HOSPITAL DAS CLÍNICAS DE TERESÓPOLIS":
                    codHospital = 3
                    py.moveTo(1111, 602)
                    py.click()
                elif caixaUnidade.get() == "AMBULATÓRIO (FATURAMENTO)":
                    codHospital = 13
                    py.moveTo(1027, 568)
                    py.click()
                elif caixaUnidade.get() == "MATERNIDADE DO HCTCO (FATURAMENTO)":
                    codHospital = 14
                    py.moveTo(819, 438)
                    py.click()

                py.moveTo(1197, 550)
                py.click()

            else:
                py.alert("A janela de Recebimentos não startou. Encerrando...")
                exit()

        except KeyboardInterrupt as kex:
            print("Programa encerrado pelo usuário via teclado.")
            exit()
        except:
            print("Resolução de tela não cadastrada no sistema")
            exit()

    else:
        py.alert("Resolução de tela não cadastrada. Encerrando...")
        exit()

    #Gerador de repetições
    conexao = cx_Oracle.connect(user, password, dsn)
    cursor = conexao.cursor()
    time.sleep(10)
    im3 = pyautogui.screenshot(f'db/image.png', region=(337,436,31,15))
    time.sleep(4)
    def reconhecimento():
        #phrase2 = ocr.image_to_string(PIL.Image.open('db/image.png'), lang='por')
        #print(int(phrase2))
        global value
        value = int(entradaOrdemFatura.get())
        global valuePeriodoMes
        valuePeriodoMes = entradaPeriodoMes.get()
        global valuePeriodoAno
        valuePeriodoAno = entradaPeriodoAno.get()
        global valuePeriodo
        valuePeriodo = str(valuePeriodoMes) + '/' + str(valuePeriodoAno)
        sql = f"SELECT count(A.NUMEROLOTE) FROM SZFATURACONVENIO A WHERE A.CODCOLIGADA = 1 AND A.CODCOMPRADOR = "+str(codPlano)+" AND A.IDUNIDFAT = "+str(codHospital)+" AND A.STATUSPROTOCOLO <> 'C' AND (TO_CHAR(A.DATAGERACAO, 'MM')|| '/' || TO_CHAR(A.DATAGERACAO, 'YYYY')) = '"+str(valuePeriodo)+"' ORDER BY A.NUMEROREMESSA ASC"
        cursor.execute(sql)
        global tables
        tables = int(cursor.fetchone()[0])
        print(f"Foram encontradas {tables} ocorrências na página de faturas.")
        sql2 = "SELECT NUMEROREMESSA FROM (SELECT ROW_NUMBER() OVER(ORDER BY NUMEROREMESSA ASC) AS Row#, CODCOLIGADA,CODCOMPRADOR, IDUNIDFAT, NUMEROLOTE, STATUSPROTOCOLO, NUMEROREMESSA FROM SZFATURACONVENIO WHERE CODCOLIGADA = 1 AND CODCOMPRADOR = "+str(codPlano)+" AND IDUNIDFAT = "+str(codHospital)+" AND STATUSPROTOCOLO <> 'C' AND (TO_CHAR(DATAGERACAO, 'MM')|| '/' || TO_CHAR(DATAGERACAO, 'YYYY')) = '"+str(valuePeriodo)+"' ) WHERE Row# = "+str(value)+" ORDER BY NUMEROREMESSA ASC"
        cursor.execute(sql2)
        global fatura
        fatura = cursor.fetchone()[0]
        print(fatura)
        #Marcar Fatura no sistema
        if value == 1:
            print("Fatura escolhida se encontra na primeira posição")
        elif value == 2:
            py.press('down')
        elif value == 3:
            py.press('down')
            py.press('down')
        elif value == 4:
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 5:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 6:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 7:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 8:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 9:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 10:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 11:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 12:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 13:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 14:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 15:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 16:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 17:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 18:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 19:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 20:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 21:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 22:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 23:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 24:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 25:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 26:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 27:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 28:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 30:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 31:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 32:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 33:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 34:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 35:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 36:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 37:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 38:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 39:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 40:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 41:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 42:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 43:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 44:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 45:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 46:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 47:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 48:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 49:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
        elif value == 50:
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')

    reconhecimento()
    value = int(entradaOrdemFatura.get())
    contador = 0
    while True:
        sql4 = f"SELECT NUMEROREMESSA FROM (SELECT ROW_NUMBER() OVER(ORDER BY NUMEROREMESSA ASC) AS Row#, CODCOLIGADA, CODCOMPRADOR, IDUNIDFAT, NUMEROLOTE, STATUSPROTOCOLO, NUMEROREMESSA FROM SZFATURACONVENIO WHERE CODCOLIGADA = 1 AND CODCOMPRADOR = "+str(codPlano)+" AND IDUNIDFAT = "+str(codHospital)+" AND STATUSPROTOCOLO <> 'C' AND (TO_CHAR(DATAGERACAO, 'MM')|| '/' || TO_CHAR(DATAGERACAO, 'YYYY')) = '"+str(valuePeriodo)+"' ) WHERE Row# = "+str(value)+" ORDER BY NUMEROREMESSA ASC"
        cursor.execute(sql4)
        faturaInt = cursor.fetchone()[0]
        print(faturaInt)
        #Tabela SZPARCIALATEND fornece os dados com total acurácia.
        sql3 = "SELECT count(*) FROM RM.SZPARCIALATEND WHERE NUMEROREMESSA = "+str(faturaInt)+" AND CODCONVENIO = "+str(codPlano)+" AND IDUNIDFAT = "+str(codHospital)+""
        cursor.execute(sql3)
        global tables3
        tables3 = int(cursor.fetchone()[0])
        print(f"{tables3} contas encontradas nessa fatura.")
        #Clique na seção de Contas
        py.moveTo(610,356)
        py.click()
        #Verificar se conta é de Recursos ou não. Apenas as que não são de Recursos devem ser baixadas
        if contador == 0:
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,452,35,13))
        elif contador == 1:
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,471,35,13))
        elif contador == 2:
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,486,35,13))
        elif contador == 3:
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,504,35,12))
        elif contador == 4:
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,520,35,13))
        elif contador == 5:
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,538,35,13))
        elif contador == 6:
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,556,35,13))
        elif contador == 7:
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,571,35,13))
        elif contador == 8:
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,590,35,13))
        elif contador == 9:
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,605,35,13))
        elif contador == 10:
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,623,35,13))
        elif contador == 11:
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 12:
            py.moveTo(515, 645)
            py.click()
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 13:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 14:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 15:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 16:
            py.moveTo(360, 640)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 16:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 17:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 18:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 19:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 20:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 21:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 22:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 23:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 24:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 25:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 26:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 27:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 28:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 29:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 30:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 31:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 32:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 33:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 34:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 35:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))
        elif contador == 36:
            py.moveTo(517, 645)
            py.click()
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            py.press('down')
            printarConta = pyautogui.screenshot(f'contas/image{contador}.png', region=(360,640,35,13))

        #Criação de rede Neural para detecção de alto nível
        image = cv2.imread(f'contas/image{contador}.png')
        width = 120
        height = 44
        image = cv2.resize(image, (width, height))
        image = cv2.fastNlMeansDenoisingColored(image, None, 10, 10, 7, 21)
        image = cv2.addWeighted(image, 1.0, np.zeros(image.shape, image.dtype), 0, 50)
        gray = cv2.Canny(image, 100, 200)
        gray = cv2.Laplacian(image, cv2.CV_64F)
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        gray = cv2.bilateralFilter(gray,9,75,75)
        gray = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
        gray = cv2.equalizeHist(gray)
        gray = cv2.GaussianBlur(gray, (1,1), 0)
        thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
        cv2.imwrite(f'except/contas/image{contador}.png', thresh)
        custom_config = r'--oem 1 --psm 6 tessedit_char_whitelist=1234567890'
        conta = ocr.image_to_string(thresh, lang='por', config=custom_config)
        print(conta)
        sql5 = "SELECT 'CONTA INICIAL' TIPO_CONTA,F.SIGLA,A.DATAENTRADA,A.DATASAIDA,B.NUMEROREMESSA  REMESSA, to_char(G.DATAGERACAO, 'DD/MM/YYY') GERACAO,E.PRONTUARIO,E.CODPACIENTE,A.CODATENDIMENTO,E.NOMEPACIENTE  PACIENTE,B.IDUNIDFAT,A.CODCOMPRADOR, A.SEQUENCIALCONTA FROM SZATENDIMENTO A INNER JOIN SZPARCIALATEND B ON  A.CODCOLIGADA = B.CODCOLIGADA  AND A.NUMEROCONTA = B.NUMEROCONTA  AND A.SEQUENCIALCONTA = B.SEQUENCIALCONTA INNER JOIN SZPACIENTE E ON  A.CODCOLIGADA = E.CODCOLIGADA  AND A.CODPACIENTE = E.CODPACIENTE INNER JOIN SZCADGERAL F ON A.CODCOLIGADA = F.CODCOLIGADA  AND B.CODCONVENIO = F.CODGERAL INNER JOIN SZFATURACONVENIO G ON A.CODCOLIGADA = G.CODCOLIGADA  AND B.CODCONVENIO = G.CODCOMPRADOR AND B.CODCOLIGADA = G.CODCOLIGADA  AND B.NUMEROREMESSA = G.NUMEROREMESSA WHERE A.CODCOLIGADA = 1 and g.numeroremessa = "+str(faturaInt)+" and g.codcomprador = "+str(codPlano)+" and a.sequencialconta = "+str(conta)+" AND A.CODCOMPRADOR NOT IN (494, 25, 3087) UNION ALL SELECT 'CONTA DE RECURSO' TIPO_CONTA, F.SIGLA, A.DATAENTRADA, A.DATASAIDA, B.NUMEROREMESSA  REMESSA, to_char(G.DATAGERACAO, 'DD/MM/YYY') GERACAO, E.PRONTUARIO, E.CODPACIENTE, A.CODATENDIMENTO,E.NOMEPACIENTE  PACIENTE, B.IDUNIDFAT, A.CODCOMPRADOR, A.SEQUENCIALCONTA FROM SZATENDIMENTOREC A INNER JOIN SZPARCIALATEND B ON  A.CODCOLIGADA = B.CODCOLIGADA     AND A.NUMEROCONTA = B.NUMEROCONTA    AND A.SEQUENCIALCONTA = B.SEQUENCIALCONTA INNER JOIN SZPACIENTE E ON  A.CODCOLIGADA = E.CODCOLIGADA  AND A.CODPACIENTE = E.CODPACIENTE INNER JOIN SZCADGERAL F ON A.CODCOLIGADA = F.CODCOLIGADA  AND B.CODCONVENIO = F.CODGERAL INNER JOIN SZFATURACONVENIO G ON A.CODCOLIGADA = G.CODCOLIGADA  AND B.CODCONVENIO = G.CODCOMPRADOR  AND B.CODCOLIGADA = G.CODCOLIGADA  AND B.NUMEROREMESSA = G.NUMEROREMESSA WHERE A.CODCOLIGADA = 1 AND B.STATUS = 'F' and g.numeroremessa = "+str(faturaInt)+" and g.codcomprador = "+str(codPlano)+" and a.sequencialconta = "+str(conta)+" AND G.STATUSPROTOCOLO = 'F' AND A.CODCOMPRADOR NOT IN (494, 25, 3087) ORDER BY PACIENTE, codpaciente, dataentrada"
        cursor.execute(sql5)
        global contaRecurso
        contaRecurso = cursor.fetchone()
        #contaRecursoSorted = sorted(contaRecurso, key=lambda x: x[9])
        tipo_conta = contaRecurso[0]
        print(tipo_conta)
        if res.width == 1366 and res.height == 768:
            #Marcar conta no sistema
            if (contador == 0) and (tipo_conta == 'CONTA INICIAL'):   
                py.moveTo(467,458)
                py.doubleClick()
            elif (contador == 1) and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(537,478)
                py.doubleClick()
            elif (contador == 2) and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(537,495)
                py.doubleClick()
            elif (contador == 3) and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(467,510)
                py.doubleClick()
            elif (contador == 4) and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(479,527)
                py.doubleClick()
            elif (contador == 5) and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(477,543)
                py.doubleClick()
            elif contador == 6 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(454, 560)
                py.doubleClick()
            elif contador == 7 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(479,578) 
                py.doubleClick()
            elif contador == 8 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(474, 596)
                py.doubleClick()
            elif contador == 9 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(469,612)
                py.doubleClick()
            elif contador == 10 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(452,627)
                py.doubleClick()
            elif contador == 11 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(466,644)
                py.doubleClick()
            elif contador == 12 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(515, 645)
                py.click()
                py.doubleClick()
            elif contador == 13 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 14 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 15 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 16 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 17 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 18 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 19 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 20 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 21 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 22 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 23 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 24  and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 25  and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 26  and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 27  and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 28  and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 29 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 30  and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 31  and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 32 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 33 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 34 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 35 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 36 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.doubleClick()
            elif contador == 37 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 38 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 39 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 40 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 41 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 42 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 43 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 44 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 45 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 46  and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 47 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 48 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 49 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 50 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 51 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 52 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 53 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 54 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 55 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 56 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 57 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 58 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 59 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 60 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 61 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 62 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 63 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 64 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 65 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 66 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 67 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 68 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 69 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 70 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 71 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 72 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 73 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 74 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 75 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 76 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 77 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 78 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 79 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 80 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 81 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 83 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 84 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(517, 645)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif tipo_conta == "CONTA DE RECURSO":
                contador += 1
                continue
            else:
                py.alert("Número de laços ainda não reconhecido.")
            title = "Registro de Recebimento por Conta"
            time.sleep(5)
            window = pygetwindow.getWindowsWithTitle(title)[0]
            if window.title == "Registro de Recebimento por Conta (RDESKAMB.UNIFESO.LAN)":
                #recebimento
                py.moveTo(479,147)
                py.click()
                title = "Informação (RDESKAMB.UNIFESO.LAN)"
                try:
                    infor = pygetwindow.getWindowsWithTitle(title)[0]
                    print(f"A janela chamada {title} está aberta.")
                    py.press('enter')
                    py.moveTo(964, 634)
                    py.click()
                    contador += 1
                    continue
                except:
                    print(f"A janela chamada {title} não saiu.")
                #Clicar na data
                py.moveTo(412, 252)
                py.click()
                im1 = pyautogui.screenshot(f'valor/image{contador}.png', region=(618,193,65,16))
                #Pre processamento de imagem usando filtros e corretivos
                image = cv2.imread(f'valor/image{contador}.png')
                width = 97
                height = 24
                image = cv2.resize(image, (width, height))
                image = cv2.fastNlMeansDenoisingColored(image, None, 10, 10, 7, 21)
                image = cv2.addWeighted(image, 1.0, np.zeros(image.shape, image.dtype), 0, 50)
                gray = cv2.Canny(image, 100, 200)
                gray = cv2.Laplacian(image, cv2.CV_64F)
                gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
                #gray = cv2.convertScaleAbs(image, alpha=1.5, beta=0)
                gray = cv2.bilateralFilter(gray,9,75,75)
                gray = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
                gray = cv2.equalizeHist(gray)
                gray = cv2.GaussianBlur(gray, (1,1), 0)
                thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
                cv2.imwrite(f"except/imageerror{contador}.png", thresh)
                time.sleep(4)
                #Criação de rede Neural para detecção de alto nível
                custom_config = r'--oem 1 --psm 6 tessedit_char_whitelist=1234567890-:.,'
                phrase1 = ocr.image_to_string(thresh, lang='por', config=custom_config)
                #Recortar valor
                py.moveTo(494, 245)
                py.click()
                py.doubleClick()
                py.press('backspace')
                #Colar em Perda
                py.moveTo(493,291)
                py.doubleClick()
                py.press('backspace')
                phrase1 = phrase1.replace(" ", "")
                locale.setlocale(locale.LC_ALL, "Portuguese_Brazil.1252")
                valor = locale.atof(phrase1)
                valor_string = str(valor)
                print(valor_string)
                py.write(valor_string.replace(".", ","), interval=0.3)
                #Gravando Processo no sistema
                py.press('tab')
                py.press('tab')
                py.press('enter')
                #Último Processo 
                py.write('1')
                py.press('enter')
                #Saindo
                py.press('enter')
                py.press('enter')
                py.press('enter')
            title = "Erro (RDESKAMB.UNIFESO.LAN)"
            try:
                time.sleep(25)
                window = pygetwindow.getWindowsWithTitle(title)[0]
                print("A tela saiu. Podemos continuar")
                py.press('enter')
            except:
                print(f"Esperando tela {title} para continuar")
                py.press('enter')
                contador += 1
                py.press('tab')
                py.press('tab')
                py.press('enter')
                continue
            #Cancelando
            py.press('tab')
            py.press('tab')
            py.press('enter')
            fim = time.time()
            resultado = (fim - inicio)
            print(f"Duração de execução até aqui foi de {resultado: .2f} segundos. E a ordem desse paciente  é a {contador}.")
            contador = contador + 1
            #Laço de Repetição externo
        if contador == tables3:
            py.moveTo(399,352)
            py.click()
            py.moveTo(349,444)
            py.click()
            py.alert("Perdas efetuadas no sistema.")
            contador = 0
            value += 1
            if value == 1:
                print("Fatura escolhida se encontra na primeira posição")
            elif value == 2:
                py.press('down')
            elif value == 3:
                py.press('down')
                py.press('down')
            elif value == 4:
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 5:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 6:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 7:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 8:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 9:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 10:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 11:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 12:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 13:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 14:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 15:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 16:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 17:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 18:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 19:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 20:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 21:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 22:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 23:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 24:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 25:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 26:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 27:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 28:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 30:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 31:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 32:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 33:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 34:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 35:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 36:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 37:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 38:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 39:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 40:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 41:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 42:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 43:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 44:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 45:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 46:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 47:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 48:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 49:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 50:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
        elif res.width == 1920 and res.height == 1080:
            #Clique na seção de Contas
            py.moveTo(832,514)
            py.click()
            #Marcar conta no sistema
            if (contador == 0) and (tipo_conta == 'CONTA INICIAL'):   
                py.moveTo(791,614)
                py.doubleClick()
            elif (contador == 1) and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(828,631)
                py.doubleClick()
            elif (contador == 2) and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(804,648)
                py.doubleClick()
            elif (contador == 3) and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840,668)
                py.doubleClick()
            elif (contador == 4) and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840,685)
                py.doubleClick()
            elif (contador == 5) and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840,701)
                py.doubleClick()
            elif contador == 6 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 715)
                py.doubleClick()
            elif contador == 7 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840,735) 
                py.doubleClick()
            elif contador == 8 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 755)
                py.doubleClick()
            elif contador == 9 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840,766)
                py.doubleClick()
            elif contador == 10 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840,784)
                py.doubleClick()
            elif contador == 11 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840,802)
                py.doubleClick()
            elif contador == 12 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.doubleClick()
            elif contador == 13 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 14 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 15 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 16 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 17 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 18 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 19 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 20 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 21 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 22 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 23 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 24  and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 25  and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 26  and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 27  and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 28  and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 29 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 30  and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 31  and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 32 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 33 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 34 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 35 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 36 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 37 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 38 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 39 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 40 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 41 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 42 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 43 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 44 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 45 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 46  and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 47 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 48 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 49 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 50 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 51 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 52 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 53 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 54 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 55 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 56 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 57 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 58 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 59 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 60 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 61 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 62 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 63 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 64 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 65 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 66 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 67 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 68 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 69 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 70 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 71 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 72 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 73 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 74 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 75 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 76 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 77 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 78 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 79 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 80 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 81 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 83 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif contador == 84 and (tipo_conta == 'CONTA INICIAL'):
                py.moveTo(840, 816)
                py.click()
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.doubleClick()
            elif tipo_conta == "CONTA DE RECURSO":
                contador += 1
                continue
            else:
                py.alert("Número de laços ainda não reconhecido.")
            title = "Registro de Recebimento por Conta"
            time.sleep(5)
            window = pygetwindow.getWindowsWithTitle(title)[0]
            if window.title == "Registro de Recebimento por Conta (RDESKAMB.UNIFESO.LAN)":
                #recebimento
                py.moveTo(750, 302)
                py.click()
                title = "Informação (RDESKAMB.UNIFESO.LAN)"
                try:
                    infor = pygetwindow.getWindowsWithTitle(title)[0]
                    print(f"A janela chamada {title} está aberta.")
                    py.press('enter')
                    py.moveTo(964, 634)
                    py.click()
                    contador += 1
                    continue
                except:
                    print(f"A janela chamada {title} não saiu.")
                #Clicar na data
                py.moveTo(626, 407)
                py.click()
                im1 = pyautogui.screenshot(f'valor/image{contador}.png', region=(880,348,65,16))
                #Pre processamento de imagem usando filtros e corretivos
                image = cv2.imread(f'valor/image{contador}.png')
                width = 97
                height = 24
                image = cv2.resize(image, (width, height))
                image = cv2.fastNlMeansDenoisingColored(image, None, 10, 10, 7, 21)
                image = cv2.addWeighted(image, 1.0, np.zeros(image.shape, image.dtype), 0, 50)
                gray = cv2.Canny(image, 100, 200)
                gray = cv2.Laplacian(image, cv2.CV_64F)
                gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
                #gray = cv2.convertScaleAbs(image, alpha=1.5, beta=0)
                gray = cv2.bilateralFilter(gray,9,75,75)
                gray = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
                gray = cv2.equalizeHist(gray)
                gray = cv2.GaussianBlur(gray, (1,1), 0)
                thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
                cv2.imwrite(f"except/imageerror{contador}.png", thresh)
                time.sleep(4)
                #Criação de rede Neural para detecção de alto nível
                custom_config = r'--oem 1 --psm 6 tessedit_char_whitelist=1234567890-:.,'
                phrase1 = ocr.image_to_string(thresh, lang='por', config=custom_config)
                #Recortar valor
                py.moveTo(779, 408)
                py.click()
                py.doubleClick()
                py.press('backspace')
                #Colar em Perda
                py.moveTo(752,446)
                py.doubleClick()
                py.press('backspace')
                phrase1 = phrase1.replace(" ", "")
                locale.setlocale(locale.LC_ALL, "Portuguese_Brazil.1252")
                valor = locale.atof(phrase1)
                valor_string = str(valor)
                print(valor_string)
                py.write(valor_string.replace(".", ","), interval=0.3)
                #Gravando Processo no sistema
                py.press('tab')
                py.press('tab')
                py.press('enter')
                #Último Processo 
                py.write('1')
                py.press('enter')
                #Saindo
                py.press('enter')
                py.press('enter')
                py.press('enter')
            title = "Erro (RDESKAMB.UNIFESO.LAN)"
            try:
                time.sleep(25)
                window = pygetwindow.getWindowsWithTitle(title)[0]
                print("A tela saiu. Podemos continuar")
                py.press('enter')
            except:
                print(f"Esperando tela {title} para continuar")
                contador -= 1
            #Cancelando
            py.press('tab')
            py.press('tab')
            py.press('enter')
            fim = time.time()
            resultado = (fim - inicio)
            print(f"Duração de execução até aqui foi de {resultado: .2f} segundos. E a ordem desse paciente  é a {contador}.")
            contador = contador + 1
            #Laço de Repetição externo
        if contador == tables3:
            py.moveTo(674,541)
            py.click()
            py.moveTo(628,597)
            py.click()
            py.alert("Perdas efetuadas no sistema.")
            contador = 0
            value += 1
            if value == 1:
                print("Fatura escolhida se encontra na primeira posição")
            elif value == 2:
                py.press('down')
            elif value == 3:
                py.press('down')
                py.press('down')
            elif value == 4:
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 5:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 6:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 7:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 8:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 9:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 10:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 11:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 12:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 13:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 14:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 15:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 16:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 17:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 18:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 19:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 20:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 21:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 22:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 23:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 24:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 25:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 26:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 27:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 28:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 30:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 31:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 32:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 33:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 34:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 35:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 36:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 37:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 38:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 39:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 40:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 41:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 42:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 43:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 44:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 45:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 46:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 47:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 48:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 49:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
            elif value == 50:
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
                py.press('down')
    else:
        py.alert("Resoluçao de tela não compatível com o sistema. Finalizando...")
        exit()

def recuperarLista():
    if Checkbutton1.get():
        baixa()
        exit()
    elif Checkbutton2.get() == 1:
        automacao()
    else:
        vazio()
        
btn = Button(window, text='Executar automação', command=recuperarLista, width=30, bg="#d3d3d3")
btn.place(x=90, y=140)

cursor.close()
window.mainloop()


